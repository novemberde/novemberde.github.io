...
Golang은 Open source project 이며 모든 개발 내역이 Github에 코드상으로 구현되어 있다.
...
"G" 는 간단하게 고루틴이다. type을 `을` 로 표현한다. 고루틴이 존재할 때, `g` 객체는
...
마지막으로, "P" 는 스케쥴러 및 메모리 할당하는 것것 같이 유저의 Go code를 설행하기 위해
